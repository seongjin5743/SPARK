...
# print(lines.collect())  # 전체 데이터를 리스트로 출력 (디버깅용)
...
# print(map_lines.collect())  # 분리된 결과 출력 (디버깅용)
...
# print(filter_lines.collect())  # 필터링된 결과 출력 (디버깅용)

# 메소드별 요청 수 계산
method_lines = map_lines.map(lambda line: (line[2], 1)) \
                        .reduceByKey(lambda a, b: a + b) \
                        .collect()
# print(method_lines)  # 메소드별 요청 수 출력

# 시간대별 요청 수 계산 (시간대는 HH:mm:ss 형식에서 HH 추출)
time_lines = map_lines.map(lambda line: (line[1].split(':')[1], 1)) \
                      .reduceByKey(lambda a, b: a + b) \
                      .collect()
# print(time_lines)  # 시간대별 요청 수 출력

# 메소드와 status별 요청 수 계산
m_s_lines = map_lines.map(lambda line: ((line[5], line[2]), 1)) \
                     .reduceByKey(lambda a, b: a + b) \
                     .collect()
...
# print(post_tuple.collect())  # 게시물 데이터 출력 (디버깅용)
